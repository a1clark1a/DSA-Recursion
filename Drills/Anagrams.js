...
For example, given "east", use "e" as a prefix and place it in front of all 6
 permutations of "ast" — "ast", "ats", "sat", "sta", "tas", and "tsa". 
...
let anagramOutput = {};
function anagramGroup(word) {
  anagram("", word);
  return Object.keys(anagramOutput);
}

const anagram = (ana = "", word) => {
  if (word === "") {
    anagramOutput[ana] = 1;
  }
  for (let i = 0; i < word.length; i++) {
    anagram(ana + word[i], word.slice(0, i) + word.slice(i + 1));
  }
};

console.log(anagramGroup("east"));
